...
Test.prototype._assert = function assert(ok, opts) {
    ...
    if (!ok && dirname !== undefined) {
        var e = new Error('exception');
        var err = $split(e.stack || '', '\n');
        var dir = dirname + path.sep;
        ...
        for (var i = 0; i < err.length; i++) {
            ...
            */
            ...
            var lineWithTokens = $replace($replace(err[i], process.cwd(), '/$CWD'), dirname, '/$TEST');
            ...
            var filePath = $replace($replace(m[2], '/$CWD', process.cwd()), '/$TEST', dirname);
